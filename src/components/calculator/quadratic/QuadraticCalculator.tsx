...
import { Copy, Check, Sparkles, HelpCircle, RefreshCw, Layers, Eye, Sliders } from "lucide-react";

type SolvingMethod = "formula" | "square" | "factoring" | "vertex";
...
export function QuadraticCalculator() {
  ...
  // Quick Presets
  const applyPreset = (preset: "real" | "repeated" | "complex" | "physics") => {
    if (preset === "real") {
      setAStr("1");
      setBStr("-5");
      setCStr("6"); // (x-2)(x-3) -> roots 2, 3
    } else if (preset === "repeated") {
      setAStr("1");
      setBStr("-6");
      setCStr("9"); // (x-3)^2 -> root 3
    } else if (preset === "complex") {
      setAStr("1");
      setBStr("2");
      setCStr("5"); // discriminant -16 -> roots -1 +/- 2i
    } else if (preset === "physics") {
      setAStr("-16");
      setBStr("64");
      setCStr("80"); // Projectile motion -> h(t) = -16t^2 + 64t + 80
    }
  };
  ...
  const calculation = useMemo(() => {
    ...
    if (a === 0) {
      // Degenerates to linear equation bx + c = 0 -> x = -c / b
      if (b === 0) {
    ...
    if (disc >= 0 && Number.isInteger(disc) && Number.isInteger(a) && Number.isInteger(b) && Number.isInteger(c)) {
      ...
      if (sqrtD * sqrtD === disc) {
        ...
        const g1 = gcd(num1, p1);
        const g2 = gcd(num2, p1);
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs text-xs">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => applyPreset("repeated")}
            className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-bold hover:bg-blue-50 dark:hover:bg-blue-950/40 hover:border-blue-300 cursor-pointer transition-colors"
          >
            Repeated Root (x²-6x+9)
          </button>
          <button
