          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-zinc-100 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-200 uppercase font-bold border-b border-zinc-200 dark:border-zinc-800">
              <tr>
                ...
                <th className="py-2.5 px-3 font-mono text-purple-700 dark:text-purple-400">% 1RM</th>
                <th className="py-2.5 px-3 font-mono text-blue-700 dark:text-blue-400">Target Weight ({result.unitLabel})</th>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60 text-zinc-700 dark:text-zinc-300">
              {result.repBreakdown.map((row, idx) => (
                <tr key={idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                  ...
                  <td className="py-2 px-3 font-mono font-bold text-purple-700 dark:text-purple-400">{row.percentage}%</td>
                  <td className="py-2 px-3 font-mono font-bold text-blue-700 dark:text-blue-400">{row.weight} {result.unitLabel}</td>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-zinc-100 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-200 uppercase font-bold border-b border-zinc-200 dark:border-zinc-800">
              <tr>
                ...
                <th className="py-2.5 px-3 font-mono text-purple-700 dark:text-purple-400">Estimated 1RM ({result.unitLabel})</th>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60 text-zinc-700 dark:text-zinc-300">
              {result.formulaResults.map((f, idx) => (
                <tr key={idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                  ...
                  <td className="py-2.5 px-3 font-mono font-bold text-purple-700 dark:text-purple-400">{f.oneRepMax} {result.unitLabel}</td>
          ...
          {result.trainingZones.map((zone, idx) => (
            <div key={idx} className="p-3.5 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-[10px] font-bold text-purple-600 dark:text-purple-400">{zone.percentageRange}</span>
              </div>
