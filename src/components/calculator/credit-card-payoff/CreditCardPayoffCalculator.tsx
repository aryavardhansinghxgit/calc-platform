        ...
        <div className="p-3 sm:p-4 space-y-3">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 items-start">
            ...
            <div className="lg:col-span-6 space-y-3">
              ...
              {/* MULTI-CARD SCHEDULE TABLE (FULL WIDTH BIGGER BOX) */}
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-xs">
                  <div className="p-3 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-2">
                    <span className="text-xs font-extrabold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                      Multi-Card Debt Roll-Down &amp; Amortization Schedule ({multiResult.totalMonths} Months Total)
                    </span>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          const headers = ["Month", "Total Payment", "Interest Paid", "Principal Paid", "Remaining Debt Balance"];
                          const rows = multiResult.schedule.map((r) => [
                            `Month ${r.month}`,
                            r.totalPayment.toFixed(2),
                            r.totalInterest.toFixed(2),
                            r.totalPrincipal.toFixed(2),
                            r.totalRemainingBalance.toFixed(2),
                          ]);
                          const csv = [headers.join(","), ...rows.map((row) => row.join(","))].join("\n");
                          triggerCsvDownload(`credit_card_payoff_schedule.csv`, csv);
                        }}
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {currentSchedulePage.map((row) => (
                          <tr key={row.month} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                            ...
                            <td className="p-2 text-blue-600 font-bold">{fmt(row.totalPrincipal)}</td>
                            <td className="p-2 font-bold text-slate-900 dark:text-slate-100">{fmt(row.totalRemainingBalance)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
