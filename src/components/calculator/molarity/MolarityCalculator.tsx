...
export function MolarityCalculator() {
  ...
  // Mass Percent & Density Inputs (Mode 3)
  const [massPercent, setMassPercent] = useState<number>(37);
  const [reagentDensity, setReagentDensity] = useState<number>(1.19);
  const [valence, setValence] = useState<number>(1);

  // PPM Converter Inputs (Mode 4)
  const [ppmValue, setPpmValue] = useState<number>(500);
  const [solventDensity, setSolventDensity] = useState<number>(1.00);
  ...
  const result: MolarityCalculatorOutputs = useMemo(() => {
    return calculateMolarityCalculator({
      ...
      v2,
      massPercent,
      densityGperML: mode === "mass_percent" ? reagentDensity : solventDensity,
      valence,
      ppm: ppmValue,
    });
  }, [
  ...
  // Live PDF Report Data Generator (reflects active state across all modes)
  const reportData: CalculatorReportData = useMemo(() => {
    ...
    if (mode === "dilution") {
      ...
      return {
        ...
        keyMetrics: [
          {
            label: "Solved Dilution Target",
            value: targetLabel,
            colorTheme: "emerald",
          },
          {
            label: "Solvent Required",
            value: `${dil?.solventVolumeNeeded.toFixed(2) || "0.00"} mL`,
            colorTheme: "blue",
          },
        ],
        sections: [
          ...
          },
          {
            ...
            items: (result.benchProtocol || []).map((step, idx) => ({
              label: `Step ${idx + 1}`,
              value: step,
            })),
          },
    ...
    if (mode === "mass_percent") {
      ...
      return {
        ...
        keyMetrics: [
          {
            label: "Calculated Molarity",
            value: `${mp?.molarityM ?? "0"} M`,
            colorTheme: "emerald",
          },
          {
            label: "Calculated Normality",
            value: `${mp?.normalityN ?? "0"} N`,
            colorTheme: "purple",
          },
        ],
        sections: [
            ...
            items: [
              {
                ...
                value: selectedCompound ? `${selectedCompound.name} (${selectedCompound.formula})` : "Custom",
              },
    ...
    if (mode === "ppm_converter") {
      ...
      return {
        ...
        keyMetrics: [
          {
            label: "Calculated Molarity",
            value: `${ppmRes?.molarityM ?? "0"} M`,
            colorTheme: "emerald",
          },
          {
            label: "Calculated Molality",
            value: `${ppmRes?.molalityM ?? "0"} mol/kg`,
            colorTheme: "teal",
          },
        ],
        sections: [
    ...
    return {
      ...
      keyMetrics: [
        {
          label: `Solved ${solveVariable.replace("_", " ").toUpperCase()}`,
          value: solvedLabel,
          colorTheme: "emerald",
        },
        {
          label: "Effective Molar Mass",
          value: `${(result.molarMass || molarMassInput).toFixed(3)} g/mol`,
          colorTheme: "blue",
        },
      ],
      sections: [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {mode === "dilution" && (
            <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs space-y-4">
              ...
              {/* Physical Domain Validation Advisory for Dilution */}
              {result.dilutionResult?.isInvalidDilution && (
                <div
                  role="alert"
                  aria-live="polite"
                  className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 text-amber-900 dark:text-amber-200 text-xs flex items-start gap-2.5"
                >
                  <Info className="h-4 w-4 text-amber-600 dark:text-amber-400 mt-0.5 shrink-0" />
                  <div>
                    <span className="font-bold block">Dilution Physical Limit Advisory</span>
                    <p className="text-[11px] leading-relaxed mt-0.5">
                      {result.dilutionResult.validationWarning}
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    type="number"
                    step="0.01"
                    value={reagentDensity}
                    onChange={(e) => setReagentDensity(parseFloat(e.target.value) || 0)}
                    className="h-9 text-xs font-sans tabular-nums bg-slate-50 dark:bg-zinc-800"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => setSolventDensity(parseFloat(e.target.value) || 1.0)}
                    className="h-9 text-xs font-sans tabular-nums bg-slate-50 dark:bg-zinc-800"
                  />
                </div>
              </div>
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-5 rounded-2xl shadow-xs space-y-4 text-slate-900 dark:text-zinc-100">
          ...
          <div className="text-center bg-gradient-to-br from-emerald-50/60 via-teal-50/40 to-blue-50/60 dark:from-zinc-800 dark:to-zinc-800/80 p-5 rounded-2xl border border-emerald-100 dark:border-zinc-700 space-y-2 shadow-xs">
            <span className="text-[10px] font-extrabold uppercase text-slate-500 dark:text-zinc-400 tracking-wider block">
              {mode === "mass_solver"
                ...
                ? solveDilutionTarget === "v1"
                  ...
                  : solveDilutionTarget === "c1"
                  ? "Calculated Stock Concentration (C₁)"
                  : solveDilutionTarget === "v2"
                  ? "Calculated Final Volume (V₂)"
                  : "Calculated Target Concentration (C₂)"
                : mode === "mass_percent"
            ...
            <div className="text-3xl font-black font-sans tabular-nums text-emerald-600 dark:text-emerald-400">
              ...
              {mode === "dilution" && result.dilutionResult && (
                <>
                  {solveDilutionTarget === "v1" && (
                    <>
                      V₁ = {result.dilutionResult.v1.toFixed(2)}{" "}
                      <span className="text-lg font-bold text-slate-700 dark:text-zinc-300">mL</span>
                    </>
                  )}
                  {solveDilutionTarget === "c1" && (
                    <>
                      C₁ = {result.dilutionResult.c1.toFixed(4)}{" "}
                      <span className="text-lg font-bold text-slate-700 dark:text-zinc-300">M</span>
                    </>
                  )}
                  {solveDilutionTarget === "v2" && (
                    <>
                      V₂ = {result.dilutionResult.v2.toFixed(2)}{" "}
                      <span className="text-lg font-bold text-slate-700 dark:text-zinc-300">mL</span>
                    </>
                  )}
                  {solveDilutionTarget === "c2" && (
                    <>
                      C₂ = {result.dilutionResult.c2.toFixed(4)}{" "}
                      <span className="text-lg font-bold text-slate-700 dark:text-zinc-300">M</span>
                    </>
                  )}
                </>
              ...
              {mode === "mass_percent" && result.massPercentResult && (
                <>
                  {result.massPercentResult.molarityM}{" "}
                  <span className="text-lg font-bold text-slate-700 dark:text-zinc-300">
                    M ({result.massPercentResult.normalityN} N)
                  </span>
                </>
              )}
            ...
            {mode === "dilution" && result.dilutionResult?.isInvalidDilution && (
              <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-md bg-amber-100 dark:bg-amber-900/60 text-amber-800 dark:text-amber-200 border border-amber-300 dark:border-amber-700 inline-block mt-1">
                Advisory: Target C₂ &gt; Stock C₁
              </span>
            )}
