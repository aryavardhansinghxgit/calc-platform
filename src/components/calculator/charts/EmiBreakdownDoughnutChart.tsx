...
export interface EmiBreakdownDoughnutChartProps {
  totalPrincipal: number;
  totalInterest: number;
  processingFee: number;
}

export function EmiBreakdownDoughnutChart({
  totalPrincipal,
  totalInterest,
  processingFee,
}: EmiBreakdownDoughnutChartProps) {
  const total = Math.max(0.01, totalPrincipal + totalInterest + processingFee);
  const principalPct = ((totalPrincipal / total) * 100).toFixed(1);
  const interestPct = ((totalInterest / total) * 100).toFixed(1);
  const feePct = ((processingFee / total) * 100).toFixed(1);
  ...
  if (processingFee > 0) {
    data.push({ name: `Processing Fee (${feePct}%)`, value: processingFee, color: "#8B5CF6" });
  }
...
export default EmiBreakdownDoughnutChart;
