...
async function runAudit() {
  console.log("Fetching http://localhost:3000/calculators/ideal-weight-calculator...\n");
  const html = await fetchUrl("http://localhost:3000/calculators/ideal-weight-calculator");

  console.log("==================================================");
  console.log("DOM & SEO FORENSIC AUDIT — IDEAL WEIGHT CALCULATOR");
  console.log("==================================================");
  ...
  const hasPdfBtn = html.includes("Generate Clinical IBW Assessment Report") || html.includes("PDF Report") || html.includes("Print");
  const hasCsvBtn = html.includes("CSV") || html.includes("Export CSV");
  const hasCopyBtn = html.includes("Copy Summary") || html.includes("Copy");
  const hasShareBtn = html.includes("Share") || html.includes("Share URL");
  const hasSaveBtn = html.includes("Save") || html.includes("Bookmark");
  const hasResetBtn = html.includes("Reset") || html.includes("Reset Defaults");
