const {
  ...
  whsRoundTenth,
} = require("../src/app/calculators/golf-handicap-calculator/calculator");
...
function runRegressionSuite() {
  ...
  // Formula: (113 / Slope) * (Score - CourseRating - 0.5 * PCC)
  // Score=40, CR=35.5, Slope=120, PCC=0 -> (113/120) * 4.5 = 4.2375 -> 4.2
  assert("9-Hole Test 9A (Score=40, CR=35.5, Slope=120, PCC=0)", calculateScoreDifferential(40, 35.5, 120, 0, 9), 4.2);
  // Score=40, CR=35.5, Slope=120, PCC=+2 -> (113/120) * (4.5 - 1.0) = 3.2958 -> 3.3
  assert("9-Hole Test 9B (Score=40, CR=35.5, Slope=120, PCC=+2)", calculateScoreDifferential(40, 35.5, 120, 2, 9), 3.3);
  // Score=40, CR=35.5, Slope=120, PCC=-1 -> (113/120) * (4.5 + 0.5) = 4.7083 -> 4.7
  ...
  const rounds4 = [...rounds3, { id: "4", score: 90.0, courseRating: 70, slopeRating: 113, pcc: 0 }];
  ...
  const rounds6 = [...rounds5, { id: "6", score: 79.8, courseRating: 70, slopeRating: 113, pcc: 0 }];
  ...
  const rounds20 = diffs20.map((d, i) => ({
    id: `r${i+1}`,
    score: 70 + d,
    courseRating: 70,
    slopeRating: 113,
  ...
  const roundsCap = Array.from({ length: 20 }, (_, i) => ({
    id: `c${i+1}`,
  ...
  const roundsHardCap = Array.from({ length: 20 }, (_, i) => ({
    id: `hc${i+1}`,
  ...
  // Baseline = 15.0. 19 rounds at diff 14.0, 1 exceptional round at diff 8.0 (gap = 7.0)
  // Lowest 8 diffs: 1 at 8.0, 7 at 14.0 -> sum = 8.0 + 98.0 = 106.0 / 8 = 13.25.
  // ESR adjustment = -1.0. Final raw index = 13.25 - 1.0 = 12.25 -> 12.3.
  const roundsEsr1 = Array.from({ length: 20 }, (_, i) => ({
    id: `esr${i+1}`,
  ...
  // Baseline = 15.0. 1 round at diff 5.0 (gap = 10.0) -> ESR = -2.0
  const roundsEsr2 = Array.from({ length: 20 }, (_, i) => ({
    id: `esr2_${i+1}`,
  ...
  // Gap = 6.9 (diff = 8.1 vs baseline 15.0) -> No ESR
  const roundsEsrNone = Array.from({ length: 20 }, (_, i) => ({
    id: `esr_none_${i+1}`,
