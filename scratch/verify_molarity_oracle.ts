...
import {
  COMMON_CHEMICAL_COMPOUNDS,
  HYDRATE_WATER_MOLAR_MASS,
} from "../src/app/calculators/molarity-calculator/compounds";
...
function assertApprox(label: string, actual: number, expected: number, tol = 1e-4) {
...
function oracleMass(M: number, V_L: number, MW: number, n_hydrate = 0) {
  const MW_eff = MW + n_hydrate * HYDRATE_WATER_MOLAR_MASS;
  return M * MW_eff * V_L;
}
function oracleMolarity(m_g: number, V_L: number, MW: number, n_hydrate = 0) {
  const MW_eff = MW + n_hydrate * HYDRATE_WATER_MOLAR_MASS;
  return m_g / (MW_eff * V_L);
}
function oracleVolume(m_g: number, M: number, MW: number, n_hydrate = 0) {
  const MW_eff = MW + n_hydrate * HYDRATE_WATER_MOLAR_MASS;
  return m_g / (M * MW_eff);
}

// Dilution: C1*V1 = C2*V2
function oracleDilution(c1: number, v1: number, c2: number, v2: number, target: string) {
...
// Mass % & Density: M = (P * rho * 10) / MW
function oracleMassPercent(P: number, rho: number, MW: number, valence = 1) {
  const M = (P * rho * 10) / MW;
  const N = M * valence;
  return { M, N };
}

// PPM to Molarity & Molality:
function oraclePPM(ppm: number, MW: number, rho = 1.0) {
